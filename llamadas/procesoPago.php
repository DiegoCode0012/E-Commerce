...
//		$id_cliente=$datos['detalle_compra']['payer']['payer_id'];
		...
		agregarPedido($conn,$transaccion,$fecha_nueva,$status,$_SESSION['usuario']['id'],$montoenSoles,$igv);
		$hola=ultimo($conn,$transaccion); //aqui hay un error , no puedo ocnseguir el ultimo id
	 	echo "string".$hola[0] . "TRANSACCION". $hola[1]; //se agregan los detalles al pedido
		if($hola[0]!=null){
